...
CREATE TABLE ws_orders (
  ...
  FOREIGN KEY (shop_id)                 REFERENCES ws_shops(id),
  -- customer_id -> client(id) : identité consolidée (ws_customers fusionnée dans client) ; pas de FK dans ce schéma
...
-- ── Back-office marque : auth, marques, communications ──────────────────────
CREATE TABLE bo_users (
  id            INT AUTO_INCREMENT PRIMARY KEY,
  email         VARCHAR(200) NOT NULL,
  password_hash VARCHAR(255) NOT NULL,
  display_name  VARCHAR(120),
  role          ENUM('siege','franchise') NOT NULL,   -- siège=réseau ; franchise=borné
...
CREATE TABLE bo_user_shops (                            -- portée par boutique (franchisé)
  user_id INT NOT NULL,
  shop_id INT NOT NULL,
  PRIMARY KEY (user_id, shop_id),
  KEY idx_bus_shop (shop_id),
  FOREIGN KEY (user_id) REFERENCES bo_users(id),
  FOREIGN KEY (shop_id) REFERENCES shops(id)            -- table unifiée
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

CREATE TABLE bo_audit (                                  -- journal actions sensibles
...
CREATE TABLE ws_brands (
  ...
  logo_media VARCHAR(255),                 -- réf bibliothèque média
  accent     VARCHAR(20) DEFAULT '#8D1D2C',
  tint       VARCHAR(20) DEFAULT '#fdf6f0',
  domain     VARCHAR(120),
  active     TINYINT(1) NOT NULL DEFAULT 1,
...
-- id_brand -> ws_brands : FK ajoutées en prod sur ws_shops.id_brand et ws_vouchers.id_brand

CREATE TABLE ws_email_templates (
  id         INT AUTO_INCREMENT PRIMARY KEY,
  tpl_key    VARCHAR(60) NOT NULL,          -- order_confirmed | order_ready | invoice | office_onboarding | join_rejected …
  lang       VARCHAR(5)  NOT NULL,          -- fr|nl|en|de
