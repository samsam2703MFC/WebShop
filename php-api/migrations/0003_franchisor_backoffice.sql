...
-- Idempotent : CREATE IF NOT EXISTS / ADD COLUMN IF NOT EXISTS (MariaDB) / seeds gardés.
...
ALTER TABLE ws_categories
  ADD COLUMN IF NOT EXISTS menu_default TINYINT NOT NULL DEFAULT 0;

ALTER TABLE ws_products
  ADD COLUMN IF NOT EXISTS menu_override   VARCHAR(8) NULL,
  ADD COLUMN IF NOT EXISTS base_cost       DECIMAL(10,2) NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS brand_mandatory TINYINT NOT NULL DEFAULT 0,  -- « Obligatoire » : verrouille l'assortiment franchisé
  ADD COLUMN IF NOT EXISTS brand_whitelist TINYINT NOT NULL DEFAULT 1;  -- « Webshop marque » : produit poussé au réseau

-- ── Gouvernance boutique (contrat + toggle Webshop par boutique) ──
ALTER TABLE ws_shops
  ADD COLUMN IF NOT EXISTS contrat         VARCHAR(16) NOT NULL DEFAULT 'Franchise',
  ADD COLUMN IF NOT EXISTS webshop_enabled TINYINT NOT NULL DEFAULT 1;

-- ── Formules : « sélection jusqu'à N » + food-cost par choix ──
ALTER TABLE ws_bundle_slots
  ADD COLUMN IF NOT EXISTS kind       VARCHAR(8) NOT NULL DEFAULT 'single',
  ADD COLUMN IF NOT EXISTS min_select INT NOT NULL DEFAULT 1,
  ADD COLUMN IF NOT EXISTS max_select INT NOT NULL DEFAULT 1;

ALTER TABLE ws_bundle_slot_choices
